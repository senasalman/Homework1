...
ggplot(tidyweather, aes(x=date, y=delta))+
  ...
  facet_wrap(~month)+
  labs (
    title = "Weather Anomalies for each month"
  )
```
...
ci_props <- social_media%>%
  group_by(sex)%>%
  summarise(sd=sd(snap_insta, na.rm = TRUE), 
            count=n(), 
            se=sqrt(p*(1-p)/n) 
            t_critical2 = qt(0.975, count-1),
            lower2 = mean-t_critical2*se,
            upper2 = mean+t_critical2*se)
