...
- German history , had a lot of wheat, started to produce beer
...
```{r, gross_on_fblikes}
glimpse(movies)
a<-ggplot(movies,aes(x=cast_facebook_likes,y=gross))+
  geom_jitter()
a
```
...
```{r, gross_on_budget}
b<-ggplot(movies,aes(x=budget,y=gross))+
  geom_jitter()
b
```
...
For most genres, the higher the ratings are, the more gross revenue a movie will make at the bax office.
...
```{r, gross_on_rating}
c<-ggplot(movies,aes(x=rating,y=gross))+
  geom_jitter()+
  facet_wrap(~genre)
c
```
...
top10_cand_city <- left_join(top10_cand_name,top_candidates,"cand_nm")%>%
  ...
  summarise(city_donation=sum(contb_receipt_amt))%>%
  top_n(10)%>%
  mutate(cand_nm=as.factor(cand_nm), primary_city = reorder_within(primary_city,city_donation,cand_nm),names)%>%
  ggplot(aes(x=city_donation, y=primary_city, fill=cand_nm))+
  geom_col(show.legend = FALSE)+
